...
@bot.command(name='clockin', aliases=['clock-in', 'ci'])
@authenticated_talento
async def clockin(ctx: commands.Context, auth: TalentaAuthData) -> None:
    with handle_errors():
        async with ctx.typing():
            if talento_auth := await Talento().authentication(auth.username, auth.password, True):
                talento = Talento(talento_auth.auth_token)
                ok, attendance_token = await talento.create_attendance('clock-in')
                if ok:
                    result = await talento.submit_attendance(attendance_token)
                    if isinstance(result, str):
                        return await direct_reply(ctx, f"got `{result}` error response when submitting clock-in attendance", True)
                    return await direct_reply(ctx, f"```json\n{json.dumps(result, indent=2)}\n```", True)
                return await direct_reply(ctx, f"got `{attendance_token}` error response when submitting clock-in attendance", True)
